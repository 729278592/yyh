...
window.onload = function () {
    var scUl = document.getElementById("active-ul");
    var aLi = scUl.getElementsByTagName("li");
    var oLeft = document.getElementById("active-left");
    var oRight = document.getElementById("active-right");
    var iNow = 1;
    var i = 0;
    var bBtn = true;
    var iTimer = null;
    ...
    function setWidth() {
        scUl.style.width = (aLi[0].offsetWidth+40) * aLi.length + 'px';
    }
    ...
    function autoPlay() {
        if (bBtn) {
            ...
            move(scUl, {left: -(aLi[0].offsetWidth+40) * iNow}, function () {
                for (i = 0; i < iNow; i++) {
    ...
    $("#active-ul,.active-left,.active-right").on("mouseover",function(){
        clearInterval(iTimer);
    });
    $(".active-u,.active-left,.active-right").on("mouseout",function(){
        iTimer = setInterval(autoPlay, 5000);
    });
};
