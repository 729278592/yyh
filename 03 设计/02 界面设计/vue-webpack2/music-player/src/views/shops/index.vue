  ...
  <div class="bd absolute pd48">
    <div class="bd absolute pd48" v-for="lists in dataJson">
        ...
        <li>
            <div>
                <div class="couponImg style">
                    <img :src="lists.personImg" class="couponImg1" alt="">
                </div>
                <div class="inforList">
                    <p class="clearfix">
                        <span class="left shopsName">{{lists.title}}</span>
                        <span class="right">{{lists.shopsType}}</span>
                    </p>
                    <div class="clearfix">
                    ...
                    <p class="clearfix">
                        <span class="left">销量{{lists.sale}}</span>
                        <span class="right">共{{lists.allNum}}件商品</span>
                    </p>
                </div>
            </div>
        </li>
      ...
      <ul class="ulmenu clearfix mt0">
          ...
          <li>
             <a v-link="'/user/detailIncome'">
                  <span class="span_index shops_icon2"></span><br/>
                  <span>收入明细</span>
              </a>
          </li>
          <li>
              <a v-link="'/user/integralComplement'">
...
<script>
  export default {
      ...
      ready () {
        document.title = '商铺主页'
        this.$http.get('../../static/data/shopsIndex.json').then(function(response){
