...
$(function(){
    $.fn.payStyle=function(options){
        var str = '<div class="mc-head">'+
            ...
            '</p>'+
            '<p>'+
            '<label><input type="checkbox" id="checked" orderId='+options.orderId+' payType='+options.payType+' /> 使用电子钱包</label> : <span class="money">￥'+options.dataAllMoney+'</span>'+                        '</p>'+
            '<button class="btn-order btn-order1" id='+options.dataId+' url='+options.href+' >去支付</button>'+
            '</div>';
