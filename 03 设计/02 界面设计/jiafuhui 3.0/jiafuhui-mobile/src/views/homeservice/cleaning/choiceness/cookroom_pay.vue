...
<div class="pay">
	<group class="group-top">
	  	<cell class="address-panel" :title="name" :inline-desc="address" ><i slot="icon" class="map icon-map-marker-o"></i></cell>
	</group>
	<div class="">
		<group>
			<div class="title_line">厨房清洗</div>
		  	<cell :value.sync="time" title="服务时间" value="2016-7-10 14:00"></cell>
		  	<cell :value.sync="number" title="清洗数量" value="3间"></cell>
...
<script>
	...
	export default {
	  ...
	  ready () {
	    ...
	    this.name = obj.name + ' ' + obj.tel
	    this.address = obj.address
	    this.number = obj.number + '间'
	    this.time = obj.year + '-' + obj.hr
