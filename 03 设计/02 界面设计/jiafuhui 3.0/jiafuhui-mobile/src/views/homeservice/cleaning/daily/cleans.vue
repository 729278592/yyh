...
<div class="clean-public month-clean" v-for="item in list">
    ...
    <div class="icebox_title">
    	<div class="left">
    		...
    		<div><span class="thumbs icon-thumbs-up"></span><rater :value=5 :margin="5" :font-size="20" :value.sync="rate" active-color="#FF5000" disabled></rater></div>
    	</div>
    ...
    <div class="bottom_btn">
    	<x-button style="background:#51AAFE;color:#fff;" v-link="'/cleaning/daily/cleans_destine'">立即预约</x-button>
    </div>
</div>
</template>
<script>
    ...
    export default {
      ...
      ready () {
        document.title = '服务详情'
        this.$progress.start()
        this.$http.get('/static/data/month_clean.json').then((response) => {
