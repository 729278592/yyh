...
<div class="destine">
	...
	<group class="area">
		<!-- 沙发 -->
	  	<x-input :value.sync="number" title="服务数量" placeholder="7" keyboard="number" v-ref:input><span>座</span></x-input>
	</group>
	<group title="更多需求：" class="textarea">
	  <x-textarea :max="200" placeholder="请输入详细描述"></x-textarea>
	</group>
	<div class="serve">服务费：<div><span>{{number * 6}}</span>.00元</div></div>
	...
	<div class="btn-panel">
      <x-button type="primary"style="background:#51AAFE;" v-link="'/cleaning/furnishing/safo_pay'">下一步</x-button>
    </div>
</div>
</template>
