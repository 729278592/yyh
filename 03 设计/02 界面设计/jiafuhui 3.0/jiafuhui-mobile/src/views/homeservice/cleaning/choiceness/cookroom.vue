...
<div class="clean-public cookroom">
    ...
    <div class="icebox_title">
    	...
    	<div class="right"><span>{{item.price}}</span>元/间</div>
    </div>
    ...
    <div class="bottom_btn">
    	<x-button style="background:#51AAFE;color:#fff;" v-link="'/cleaning/choiceness/cookroom_destine'">立即预约</x-button>
    </div>
</div>
</template>
<script>
    ...
    export default {
      ...
      ready () {
        document.title = '服务详情'
        this.$progress.start()
        this.$http.get('/static/data/cookroom.json').then((response) => {
