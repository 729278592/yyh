...
<div class="clean-public kitchen">
    ...
    <div class="content_range white_bg">
      ...
      <p class="content_text">{{ item.content }}</p>
    	<p class="content_text" style="text-indent: 0;padding:20px 30px;">{{{ item.content_img }}}</p>
    </div>
    <div class="bottom_btn">
    	<x-button style="background:#51AAFE;color:#fff;" v-link="'/cleaning/appliance/kitchen_destine'">立即预约</x-button>
    </div>
</div>
</template>
<script>
    ...
    export default {
      ...
      ready () {
        document.title = '服务详情'
        this.$progress.start()
        this.$http.get('/static/data/kitchen.json').then((response) => {
