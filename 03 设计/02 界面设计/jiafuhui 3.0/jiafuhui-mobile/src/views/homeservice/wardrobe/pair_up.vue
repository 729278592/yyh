...
<div class="shine-shoes clean-public" style="padding-bottom:60px;" v-for="item in list">
    ...
    <div class="icebox_title">
      ...
      <div class="right"><span>{{item.price}}</span>元/套</div>
    </div>
    ...
    <div class="bottom_btn">
    	<x-button style="background:#51AAFE;color:#fff;" v-link="'/wardrobe/pair_up_pay'">立即订购</x-button>
    </div>
</div>
</template>
...
export default {
  ...
  ready () {
    document.title = '服务详情'
    this.$progress.start()
    this.$http.get('/static/data/pair_up.json').then((response) => {
