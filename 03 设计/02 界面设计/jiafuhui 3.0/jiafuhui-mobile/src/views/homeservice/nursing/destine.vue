...
<div class="destine">
	...
	<div class="hospital-care" style="padding-bottom: 10px;"> <!-- v-if="checked1==true" -->
		...
		<flexbox class="group-top" style="margin:0;padding: 10px 0;">
			...
			<flexbox-item class="vux-1px-r">
				<div class="flex-demo"><i slot="icon" class="edit icon-edit"></i><x-input class="edit-num" title="" placeholder="填写病房号"></x-input></div>
			</flexbox-item>
		</flexbox>
	</div>
	<!-- 到家护理 -->
	...
	<group class="cell">
		<cell title="已预定" value="一级护工"></cell>
	  	<x-input :value.sync="area" title="护理天数" placeholder="2" keyboard v-ref:input><span>天</span></x-input>
	</group>
	<group class="cell">
		<cell title="紧急联系人" value="张阿姨  18679980669"></cell>
	</group>
	<group title="更多需求：" class="textarea">
	  <x-textarea :max="200" placeholder="请输入详细描述"></x-textarea>
	</group>
	<div class="serve">服务费：<div><span>{{area}}</span>.00元</div></div>
	...
	<div class="btn-panel">
      <x-button type="primary"style="background:#51AAFE;" v-link="'/nursing/pay'">下一步</x-button>
    </div>
</div>
</template>
