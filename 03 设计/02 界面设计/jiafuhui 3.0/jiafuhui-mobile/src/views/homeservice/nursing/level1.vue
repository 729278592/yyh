...
<div class="nursing_level" v-for="item in list">
    ...
    <div class="list">
        <flexbox>
          	<flexbox-item v-link=""><div class="item item-active">一级护工</div></flexbox-item>
          	<flexbox-item v-link="'/nursing/level2'"><div class="item">二级护工</div></flexbox-item>
          	<flexbox-item v-link="'/nursing/level3'"><div class="item">三级护工</div></flexbox-item>
        </flexbox>
    </div>
    ...
    <div class="content_range white_bg">
    	...
    	<p class="content_text" style="text-indent: 0;padding:20px 30px;" v-if="tab_index == 0"><img width="100%" src="/static/temp/nursing/level1_01.jpg"></p>
    	<p class="content_text" v-if="tab_index == 1">雾化辅助、扣背排痰、鼻饲辅助、痰口护理</p>
    </div>
    <div class="content_range white_bg">
    ...
    <div class="bottom_btn">
    	<x-button style="background:#51AAFE;color:#fff;" v-link="'/nursing/destine1'">立即预约</x-button>
    </div>
</div>
</template>
<script>
  ...
  export default {
    ...
    ready () {
      document.title = '服务详情'
      this.$progress.start()
      this.$http.get('/static/data/level1.json').then((response) => {
