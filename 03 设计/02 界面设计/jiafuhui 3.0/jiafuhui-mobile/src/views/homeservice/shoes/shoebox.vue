<template>
<div class="shine-shoes clean-public" style="padding-bottom:110px;" v-for="item in list">
	  <div class="header">
        <div class="area_title"><span>鞋具<small>护理</small></span></div>
    </div>
    <div class="icebox_title">
      <div class="left">
        <div class="top">鞋柜清理</div>
      ...
      <div class="right"><span>{{item.price}}</span>元/米<small style="color:#bbb;text-decoration:line-through;">5.00元</small></div>
    </div>
    ...
    <div class="content_range white_bg">
        ...
        <p class="content_text col">{{{item.flow_path}}}</p>
    </div>
    
    <div class="bottom_btn">
    	<x-button style="background:#51AAFE;color:#fff;" v-link="'/shoes/shoebox_pay'">在线预约</x-button>
    </div>
</div>
</template>
...
export default {
  ...
  ready () {
    document.title = '服务详情'
    this.$progress.start()
    this.$http.get('/static/data/shoebox.json').then((response) => {
