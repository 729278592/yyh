...
<div class="destine">
	<group class="group-top">
	  	<cell title="点击设置或添加地址" v-link="'../../user/address'" is-link><i slot="icon" class="map icon-map-marker-o"></i></cell>
	</group>
	<flexbox>
	...
	<group class="area">
		<!--催乳师显示-->
	  	<x-input :value.sync="times" title="服务次数" placeholder="1次起订" keyboard v-ref:input><span>次</span></x-input>
	  	<!--入户指导显示-->
	  	<x-input :value.sync="duration" title="服务时长" placeholder="3小时起订" keyboard v-ref:input><span>小时</span></x-input>
	</group>	
	<group title="更多需求：" class="textarea">
	  <x-textarea :max="200" placeholder="请输入详细描述"></x-textarea>
	</group>
	<div class="serve">服务费：<div><span>{{area * 60}}</span>.00元</div></div>
	...
	<div class="btn-panel">
      <x-button type="primary"style="background:#51AAFE;" v-link="'/motherbaby/pay'">下一步</x-button>
    </div>
</div>
</template>
