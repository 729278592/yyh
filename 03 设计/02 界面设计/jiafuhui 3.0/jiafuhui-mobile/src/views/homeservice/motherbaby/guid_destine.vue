...
<div class="destine">
	...
	<group class="area">
	  	<x-input :value.sync="duration" title="服务时长" placeholder="3小时起订" keyboard="number" v-ref:input><span>小时</span></x-input>
	</group>	
	<group title="更多需求：" class="textarea">
	  <x-textarea :max="200" placeholder="请输入详细描述"></x-textarea>
	</group>
	<div class="serve">服务费：<div><span>{{duration * 60}}</span>.00元</div></div>
	...
	<div class="btn-panel">
      <x-button type="primary"style="background:#51AAFE;" v-link="'/motherbaby/guid_pay'">下一步</x-button>
    </div>
</div>
</template>
